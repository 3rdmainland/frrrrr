...
class NavigationService {
  ...
  getNavigation() {
    return Cache.get('navigation') ||
      Cache.put('navigation', ApiHttp.get(`/navigation`)
        .then(response => {
...
export class NavigationItem {
  ...
  constructor(data) {
    ...
    this.path = data.path
    this.isExternal = ! this.path.startsWith(process.env.VUE_APP_ROUTER_BASE_PATH)
    this.children = data.children && data.children.map(child => new NavigationItem(child))
    this.color = data.color
    this.icon = data.icon

    if(!this.isExternal) this.internalPath = this.path.replace(process.env.VUE_APP_ROUTER_BASE_PATH, '')
  }
}
