<template>
  <div v-if="showSection" class="specials-section mb-6">
    <NMenuSection title="Specials" buttonText="All Specials" :buttonAction="navigateToAllSpecials" />
    ...
    <section class="menu-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      <template v-else-if="error">
        <div class="col-span-full p-4 bg-yellow-100 text-yellow-800 rounded">
          <p class="font-semibold">Unable to load specials</p>
...
const navigateToAllSpecials = () => {
  router.push('/menu/specials')
}

const forceRefresh = async () => {
  menuStore.clearCache('special', true)
  await loadItems()
}
...
const handleAddToBasket = async (idPath) => {
  ...
  try {
    ...
    emit('error', err)
    console.error('[SpecialsSection] Failed to add item:', err)
  } finally {
...
const loadItems = async () => {
  ...
  try {
    const specialItems = await menuStore.getSpecialItems(6)
    console.log('[SpecialsSection] Loaded items:', specialItems.length)

    items.value = specialItems.map((item) => ({
      title: item.name,
      description: item.description || '',
    ...
    console.error('[SpecialsSection] Failed to load items:', err)
    error.value = err
    emit('error', err)
  } finally {
